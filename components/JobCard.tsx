...
const JobCard: React.FC<JobCardProps> = ({ job, onUpdate, onDiscard }) => {
    ...
    <div className="bg-white border border-gray-100 rounded-2xl p-5 flex flex-col h-[400px] shadow-sm hover:shadow-md hover:border-gray-200 transition-all group relative">
      ...
      <div className="flex justify-between items-start mb-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div className="w-9 h-9 bg-gray-50 border border-gray-100 rounded-lg flex items-center justify-center text-red-500 group-hover:bg-black group-hover:text-white transition-colors flex-shrink-0">
            <svg
              className="w-4 h-4"
              fill="none"
              stroke="currentColor"
          ...
          <div className="flex-1 min-w-0">
            {isEditingEmail ? (
              ...
              <div
                ...
                className="cursor-pointer hover:bg-gray-50 rounded px-1 py-0.5 -mx-1 -my-0.5 transition"
              >
        ...
        <div className="flex items-center gap-2">
          ...
          <button>
            {onDiscard && job.status === "completed" && (
            <button
              onClick={onDiscard}
              className="px-2 py-0.5 w-7 h-7 flex items-center justify-center rounded-full hover:bg-red-50 text-gray-400 hover:text-red-500 transition-all z-30 hover:opacity-100"
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {job.status === "completed" ? (
          <div className="flex-1 flex flex-col space-y-3">
            <div>
              <span className="text-[9px] font-bold text-gray-300 uppercase tracking-widest block mb-0.5">
                Subject
              </span>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 mb-2">
              <p className="text-[9px] text-blue-800 font-medium leading-relaxed">
                <span className="font-bold">Note:</span> Add your profile
                details (name, email, phone, etc.) to get personalized email
                drafts with your information in the header.
              </p>
            </div>
